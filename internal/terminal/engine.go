...
func (e *Engine) Spawn(ctx context.Context, opts SpawnOpts) (types.TerminalSession, error) {
	...
	lv := &live{meta: meta, cmd: cmd, pty: ptmx, cancel: cancel}
	e.mu.Lock()
	e.sess[meta.ID] = lv
	e.mu.Unlock()
	if e.store != nil {
		...
		if n > 0 {
			...
			if e.bus != nil {
				e.bus.Publish(types.Event{
					Type:  types.EventTerminalData,
					Topic: "terminal." + string(lv.meta.ID),
					Payload: map[string]any{
						"id":   string(lv.meta.ID),
						"data": string(chunk),
					},
				})
			}
...
func (e *Engine) wait(lv *live) {
	...
	if e.bus != nil {
		e.bus.Publish(types.Event{
			Type:  types.EventTerminalExit,
			Topic: "terminal." + string(meta.ID),
			Payload: map[string]any{
				"id":       string(meta.ID),
				"exitCode": code,
			},
...
func (e *Engine) Resize(id types.ID, cols, rows int) error {
	...
	lv.meta.Rows = rows
	lv.mu.Unlock()
	return pty.Setsize(lv.pty, &pty.Winsize{Cols: uint16(cols), Rows: uint16(rows)})
}
...
func (e *Engine) Restart(ctx context.Context, id types.ID) (types.TerminalSession, error) {
	...
	opts := SpawnOpts{
		Kind:        lv.meta.Kind,
		OwnerID:     lv.meta.OwnerID,
		WorkspaceID: lv.meta.WorkspaceID,
		Cwd:         lv.meta.Cwd,
		Shell:       lv.meta.Shell,
		Cols:        lv.meta.Cols,
		Rows:        lv.meta.Rows,
		Title:       lv.meta.Title,
		Persistent:  lv.meta.Persistent,
		SSH:         lv.meta.SSH,
	}
	lv.mu.Unlock()
	_ = e.Kill(id)
